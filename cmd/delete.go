...
var deleteCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			fmt.Printf("⚠️ Warning: Could not read public key for '%s'. Remote deletion may require manual cleanup: %v\n", accountName, err)
		}

		fmt.Println("\n[1/4] Authenticating to delete public key from GitHub...")
		...
		fmt.Println("\n[2/4] Deleting registered SSH public key from GitHub...")
		if publicKey != "" {
			err = github.DeleteSSHPublicKey(accessToken, publicKey)
		}
		...
		fmt.Println("\n[3/4] Removing local SSH keys and configuration...")
